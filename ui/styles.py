...
def render_global_styles(colors: dict[str, str]) -> None:
    ...
    st.markdown(
        f"""
        <style>
        :root {{ --bg:{colors['background']}; --app-bg:{colors['background']}; --surface:{colors['surface']}; --surface-secondary:{colors['secondary']}; --surface-elevated:{colors['elevated']}; --surface-raised:{colors['elevated']}; --elevated:{colors['elevated']}; --text:{colors['text']}; --primary-text:{colors['text']}; --text-secondary:{colors['secondary_text']}; --muted:{colors['muted']}; --muted-text:{colors['muted']}; --border:{colors['border']}; --hover:{colors['hover']}; --accent:{colors['accent']}; --icon-primary:{colors['icon_primary']}; --icon-secondary:{colors['icon_secondary']}; --icon-accent:{colors['icon_accent']}; --button-secondary-text:{colors['button_secondary_text']}; --button-primary-bg:{'#47555F' if dark_theme else '#192B37'}; --button-primary-hover:{'#5E6A73' if dark_theme else '#30404B'}; --button-primary-disabled-bg:{'#30404B' if dark_theme else '#E8EAEB'}; --button-primary-disabled-text:{'#BABFC3' if dark_theme else '#8C959B'}; --button-secondary-hover:{'#30404B' if dark_theme else '#E8EAEB'}; --button-secondary-hover-border:{'#47555F' if dark_theme else '#BABFC3'}; --action-hover:{'#47555F' if dark_theme else '#FFE8E3'}; --auth-toggle-bg:{'transparent' if dark_theme else '#FFFFFF'}; --auth-toggle-hover:{'#30404B' if dark_theme else '#E8EAEB'}; --auth-toggle-hover-border:{'#47555F' if dark_theme else '#D1D5D7'}; --input-bg:{colors['input_bg']}; --input-hover:{colors['input_hover']}; --input-hover-border:{'#5E6A73' if dark_theme else '#BABFC3'}; --input-border:{colors['input_border']}; --input-text:{colors['input_text']}; --input-placeholder:{colors['input_placeholder']}; --input-focus:{colors['input_focus']}; --focus-border:{colors['input_focus']}; --dropdown-bg:{colors['input_bg']}; --dropdown-text:{colors['input_text']}; --disabled-bg:{colors['disabled_bg']}; --disabled-text:{colors['disabled_text']}; --spinner-active:{colors['spinner_active']}; --spinner-track:{colors['spinner_track']}; --table-distinct:{table_distinct}; --table-range:{table_range}; --table-sample:{table_sample}; --source-blue:{source_blue}; --source-postgres:{source_postgres}; }}
        html, body, [data-testid="stAppViewContainer"], [data-testid="stAppViewBlockContainer"], .stApp {{ color-scheme:{'dark' if dark_theme else 'light'}; }}
        .stApp {{ background:var(--app-bg); color:var(--primary-text); }}
        ...
        .html-table-wrapper--scrollable .html-table-scroll {{ overflow-y:auto; scrollbar-color:var(--border) transparent; scrollbar-width:thin; }}
        .html-table-wrapper--sticky-header .html-table thead th {{ background:var(--elevated); position:sticky; top:0; z-index:2; }}
        .html-table-wrapper--compact {{ max-width:none; width:100%; }}
        .html-table-wrapper--compact .html-table {{ min-width:0; table-layout:fixed; }}
        .html-table-wrapper--compact .html-table-cell--identifier {{ min-width:0; }}
        .html-table-wrapper--compact .html-table-cell {{ overflow-wrap:anywhere; white-space:normal; padding-bottom:.52rem; padding-top:.52rem; }}
        @media (max-width:800px) {{ .html-table-wrapper--compact {{ max-width:100%; }} }}
        [class*="st-key-table-shell-"] {{ position:relative; }}
        [class*="st-key-table-shell-"] [data-testid="stHorizontalBlock"] {{ align-items:flex-start; background:transparent!important; border:0!important; box-shadow:none!important; display:flex; filter:none!important; justify-content:flex-end; opacity:0; pointer-events:none; position:absolute; right:.45rem; top:.45rem; transform:none!important; transition:opacity 120ms ease, visibility 120ms ease; visibility:hidden; width:auto!important; z-index:20; }}
        [class*="st-key-table-shell-"] [data-testid="stHorizontalBlock"] *, [class*="st-key-table-shell-"] [data-testid="stDownloadButton"], [class*="st-key-table-shell-"] [data-testid="stDownloadButton"] * {{ background:transparent!important; border:0!important; box-shadow:none!important; filter:none!important; transform:none!important; }}
        [class*="st-key-table-shell-"]:hover [data-testid="stHorizontalBlock"] {{ opacity:1; pointer-events:auto; visibility:visible; }}
        [class*="st-key-table-shell-"] [data-testid="stDownloadButton"] button {{ background:var(--surface); border:1px solid var(--border); box-shadow:none!important; color:var(--icon-secondary); height:30px; min-height:28px; min-width:30px; padding:0 .25rem; width:30px; }}
        [class*="st-key-table-shell-"] [data-testid="stDownloadButton"] button:hover {{ background:var(--input-hover); border-color:var(--border); box-shadow:none!important; color:var(--accent); transform:none; }}
        ...
        .html-table--fill-available {{ min-width:max-content; table-layout:auto; width:100%; }}
        .html-table.failed-records, .html-table.failed-records-with-reason {{ min-width:max-content; table-layout:auto; width:100%; }}
        .html-table.failed-records th, .html-table.failed-records td, .html-table.failed-records-with-reason th, .html-table.failed-records-with-reason td {{ min-width:8rem; white-space:nowrap; }}
        .html-table.failed-records-with-reason th:last-child, .html-table.failed-records-with-reason td:last-child {{ max-width:32rem; min-width:22rem; white-space:normal; overflow-wrap:anywhere; }}
        [class*="st-key-quality-failed-records-download"] [data-testid="stDownloadButton"], [class*="st-key-quality-all-failed-records-download"] [data-testid="stDownloadButton"] {{ width:max-content!important; }}
        [class*="st-key-quality-failed-records-download"] [data-testid="stDownloadButton"] button, [class*="st-key-quality-all-failed-records-download"] [data-testid="stDownloadButton"] button {{ min-width:max-content; white-space:nowrap; width:auto; }}
        ...
        [data-baseweb="input"], [data-baseweb="base-input"], [data-baseweb="select"], [data-baseweb="textarea"], [data-baseweb="input"] > div, [data-baseweb="base-input"] > div, [data-baseweb="select"] > div, [data-baseweb="textarea"] > div {{ background:var(--input-bg)!important; background-color:var(--input-bg)!important; border-color:var(--input-border)!important; color:var(--input-text)!important; }}
        [data-testid="stTextInput"] [data-baseweb="input"], [data-testid="stTextInput"] [data-baseweb="base-input"], [data-testid="stNumberInput"] [data-baseweb="input"], [data-testid="stNumberInput"] [data-baseweb="base-input"], [data-testid="stTextArea"] [data-baseweb="textarea"], [data-testid="stTextArea"] [data-baseweb="base-input"] {{ background:var(--input-bg)!important; background-color:var(--input-bg)!important; border-color:var(--input-border)!important; color:var(--input-text)!important; }}
        [data-baseweb="input"]:hover, [data-baseweb="base-input"]:hover, [data-baseweb="select"]:hover, [data-baseweb="textarea"]:hover {{ background:var(--input-hover)!important; background-color:var(--input-hover)!important; border-color:var(--input-hover-border)!important; }}
        [data-baseweb="input"] input, [data-baseweb="base-input"] input, [data-baseweb="textarea"] textarea, [data-baseweb="base-input"] textarea {{ background:transparent!important; background-color:transparent!important; color:var(--input-text)!important; caret-color:var(--input-focus)!important; -webkit-text-fill-color:var(--input-text)!important; }}
        [data-baseweb="input"] input::placeholder, [data-baseweb="textarea"] textarea::placeholder {{ color:var(--input-placeholder); opacity:1; }}
        [data-baseweb="input"]:focus-within, [data-baseweb="base-input"]:focus-within, [data-baseweb="select"]:focus-within, [data-baseweb="textarea"]:focus-within {{ border-color:var(--focus-border)!important; box-shadow:0 0 0 1px var(--focus-border)!important; }}
        [data-testid="stSelectbox"] [data-baseweb="select"], [data-testid="stSelectbox"] [data-baseweb="select"] > div {{ background:var(--input-bg)!important; background-color:var(--input-bg)!important; border-color:var(--input-border)!important; color:var(--input-text)!important; }}
        [data-testid="stSelectbox"] [data-baseweb="select"]:hover, [data-testid="stSelectbox"] [data-baseweb="select"]:hover > div {{ background:var(--input-hover)!important; background-color:var(--input-hover)!important; border-color:var(--input-hover-border)!important; }}
        [data-baseweb="select"] svg {{ color:var(--text-secondary)!important; fill:currentColor; }}
        [data-testid="stSelectbox"] [role="group"] {{ background:var(--input-bg)!important; background-color:var(--input-bg)!important; border-color:var(--input-border)!important; color:var(--input-text)!important; }}
        [data-testid="stSelectbox"] [role="group"] input[role="combobox"] {{ background:transparent!important; background-color:transparent!important; color:var(--input-text)!important; -webkit-text-fill-color:var(--input-text)!important; caret-color:var(--input-focus)!important; }}
        [data-testid="stSelectbox"] [role="group"]:hover {{ background:var(--input-hover)!important; background-color:var(--input-hover)!important; border-color:var(--input-hover-border)!important; }}
        [data-testid="stSelectbox"] [role="group"]:focus-within {{ border-color:var(--input-focus)!important; box-shadow:0 0 0 1px var(--input-focus)!important; }}
        [data-testid="stSelectbox"] [role="group"] svg, [data-testid="stSelectbox"] [role="group"] button {{ color:var(--text-secondary)!important; fill:currentColor; stroke:currentColor; }}
        [data-baseweb="popover"], [data-baseweb="menu"], [role="listbox"], [role="option"] {{ background:var(--dropdown-bg)!important; color:var(--dropdown-text)!important; }}
        [data-baseweb="popover"] [role="option"]:hover, [data-baseweb="popover"] [aria-selected="true"], [data-baseweb="menu"] [role="option"]:hover, [data-baseweb="menu"] [aria-selected="true"], [role="option"]:hover, [role="option"][aria-selected="true"] {{ background:var(--input-hover)!important; color:var(--dropdown-text)!important; }}
        [data-baseweb="input"] input:-webkit-autofill, [data-baseweb="base-input"] input:-webkit-autofill, [data-baseweb="textarea"] textarea:-webkit-autofill, [data-baseweb="base-input"] textarea:-webkit-autofill {{ -webkit-text-fill-color:var(--input-text)!important; box-shadow:0 0 0 1000px var(--input-bg) inset!important; caret-color:var(--input-text)!important; }}
        [data-testid="stTextInputRootElement"] {{ background:var(--input-bg)!important; background-color:var(--input-bg)!important; border-color:var(--input-border)!important; color:var(--input-text)!important; }}
        [data-testid="stTextInputRootElement"] input {{ background:transparent!important; background-color:transparent!important; color:var(--input-text)!important; -webkit-text-fill-color:var(--input-text)!important; caret-color:var(--input-focus)!important; }}
        [data-testid="stTextInputRootElement"] input::placeholder {{ color:var(--input-placeholder)!important; opacity:1!important; }}
        [data-testid="stTextInputRootElement"]:hover {{ background:var(--input-hover)!important; background-color:var(--input-hover)!important; border-color:var(--input-hover-border)!important; }}
        [data-testid="stTextInputRootElement"]:focus-within {{ border-color:var(--focus-border)!important; box-shadow:0 0 0 1px var(--focus-border)!important; }}
        [data-testid="stTextInputRootElement"] button, [data-testid="stTextInputRootElement"] svg {{ color:var(--text-secondary)!important; fill:currentColor; stroke:currentColor; }}
        ...
        .card-decoration-data-nodes,.card-decoration-data-grid,.card-decoration-quality-signal {{ overflow:hidden; position:relative; }} .card-decoration-data-nodes::before,.card-decoration-data-nodes::after,.card-decoration-data-grid::before,.card-decoration-data-grid::after,.card-decoration-quality-signal::before,.card-decoration-quality-signal::after {{ content:""; pointer-events:none; position:absolute; user-select:none; z-index:0; }} .card-content {{ position:relative; z-index:1; }} .card-decoration-data-nodes::before {{ background:linear-gradient(145deg,transparent 45%,color-mix(in srgb, var(--text-secondary) 6%, transparent) 46% 47%,transparent 48%),linear-gradient(35deg,transparent 57%,color-mix(in srgb, var(--text-secondary) 5%, transparent) 58% 59%,transparent 60%); height:72px; opacity:.75; right:8px; top:10px; width:92px; }} .card-decoration-data-nodes::after {{ background:radial-gradient(circle at 18% 68%,color-mix(in srgb, var(--accent) 12%, transparent) 0 4px,transparent 5px),radial-gradient(circle at 57% 30%,color-mix(in srgb, var(--text-secondary) 9%, transparent) 0 4px,transparent 5px),radial-gradient(circle at 84% 67%,color-mix(in srgb, var(--accent) 10%, transparent) 0 3px,transparent 4px); height:72px; right:8px; top:10px; width:92px; }} .card-decoration-data-grid::before {{ background:linear-gradient(90deg,color-mix(in srgb, var(--text-secondary) 5%, transparent) 1px,transparent 1px),linear-gradient(color-mix(in srgb, var(--text-secondary) 5%, transparent) 1px,transparent 1px); background-size:18px 18px; height:76px; opacity:.72; right:10px; top:12px; width:78px; }} .card-decoration-data-grid::after {{ background:radial-gradient(circle at 24% 26%,color-mix(in srgb, var(--accent) 10%, transparent) 0 3px,transparent 4px),radial-gradient(circle at 70% 72%,color-mix(in srgb, var(--accent) 8%, transparent) 0 3px,transparent 4px); height:76px; right:10px; top:12px; width:78px; }} .card-decoration-quality-signal::before {{ border:1px solid color-mix(in srgb, var(--text-secondary) 8%, transparent); border-radius:50%; height:34px; right:20px; top:20px; width:34px; }} .card-decoration-quality-signal::after {{ background:linear-gradient(155deg,transparent 38%,color-mix(in srgb, var(--accent) 11%, transparent) 39% 42%,transparent 43%),linear-gradient(25deg,transparent 55%,color-mix(in srgb, var(--text-secondary) 6%, transparent) 56% 58%,transparent 59%); bottom:18px; height:34px; right:8px; width:94px; }} .documentation-count-breakdown.card-decoration-data-grid::before {{ height:110px; right:20px; top:18px; width:118px; }} .documentation-count-breakdown.card-decoration-data-grid::after {{ height:110px; right:20px; top:18px; width:118px; }} .documentation-count-breakdown.card-decoration-data-nodes::before {{ height:110px; right:18px; top:18px; width:120px; }} .documentation-count-breakdown.card-decoration-data-nodes::after {{ height:110px; right:18px; top:18px; width:120px; }} @media (max-width:600px) {{ .card-decoration-data-nodes::before,.card-decoration-data-nodes::after,.card-decoration-data-grid::before,.card-decoration-data-grid::after {{ transform:scale(.78); transform-origin:top right; }} .card-decoration-quality-signal::before,.card-decoration-quality-signal::after {{ opacity:.65; }} }} [class*="st-key-reports-quality-trend"] {{ overflow:hidden; position:relative; }} [class*="st-key-reports-quality-trend"]::after {{ background:linear-gradient(155deg,transparent 38%,color-mix(in srgb, var(--accent) 7%, transparent) 39% 41%,transparent 42%),linear-gradient(25deg,transparent 58%,color-mix(in srgb, var(--text-secondary) 4%, transparent) 59% 60%,transparent 61%); content:""; height:84px; opacity:.65; pointer-events:none; position:absolute; right:14px; top:12px; user-select:none; width:120px; z-index:0; }} [class*="st-key-reports-quality-trend"] > div {{ position:relative; z-index:1; }}
        ...
        [class*="st-key-quality-view-all-failed-records"] button[kind="tertiary"] {{ justify-content:space-between; min-width:100%!important; padding:0 .75rem!important; white-space:nowrap; width:100%!important; }}
        [class*="st-key-quality-view-all-failed-records"] [data-testid="stMarkdownContainer"] {{ white-space:nowrap; }}
        [class*="st-key-quality-view-failed-records-"] button[kind="tertiary"] {{ justify-content:space-between; min-width:100%!important; padding:0 .75rem!important; white-space:nowrap; width:100%!important; }}
        [class*="st-key-quality-view-failed-records-"] [data-testid="stMarkdownContainer"] {{ white-space:nowrap; }}
        ...
        .metadata-evolution-trends {{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:1rem; margin:1rem 0 1.25rem; }}
        .metadata-evolution-trend-card {{ background:var(--surface); border:1px solid var(--border); border-radius:10px; min-height:250px; padding:.85rem 1rem; }}
        .metadata-evolution-trend-title {{ color:var(--text-secondary); font-size:.68rem; font-weight:800; letter-spacing:.08em; margin-bottom:.35rem; }}
        .metadata-trend-svg {{ display:block; height:auto; max-width:100%; width:100%; }}
        .metadata-trend-grid {{ stroke:var(--border); stroke-width:1; opacity:.65; }}
        .metadata-trend-axis {{ fill:var(--muted); font-family:inherit; font-size:10px; }}
        .metadata-trend-line {{ stroke:var(--accent); stroke-linecap:round; stroke-linejoin:round; stroke-width:3; }}
        .metadata-trend-point {{ fill:var(--surface); stroke:var(--accent); stroke-width:3; }}
        .metadata-trend-bar {{ fill:var(--accent); opacity:.22; }}
        @media (max-width:800px) {{
            ...
            .metadata-evolution-trends {{ grid-template-columns:1fr; }}
        }}
        ...
        [class*="st-key-html-table-interactive-"]:not([class*="st-key-html-table-interactive-row-"]) [data-testid="stHorizontalBlock"] {{ align-items:center; gap:0; margin:0!important; min-width:900px; width:100%; }}
        [class*="st-key-html-table-interactive-"] [data-testid="stHorizontalBlock"] > div {{ align-items:center; align-self:stretch; border-left:1px solid color-mix(in srgb, var(--border) 45%, transparent); box-sizing:border-box; display:flex; margin:0!important; min-height:0; min-width:0; padding:0 .9rem; }}
        [class*="st-key-html-table-interactive-"] [data-testid="stHorizontalBlock"] > div > [data-testid="stVerticalBlock"] {{ margin:0!important; min-height:0; padding-bottom:0!important; padding-top:0!important; }}
        ...
        .html-table-interactive-header {{ align-items:center; box-sizing:border-box; color:var(--text); display:flex; font-size:.74rem; font-weight:750; line-height:1.35; min-height:46px; padding:10px .08rem; width:100%; white-space:nowrap; }}
        .html-table-interactive-cell {{ align-items:center; box-sizing:border-box; color:var(--text); display:flex; font-size:.78rem; height:100%; line-height:1.35; min-height:0; padding:.56rem 0 .64rem; vertical-align:middle; width:100%; }}
        .html-table-interactive-cell--special {{ padding-bottom:.62rem; padding-top:.62rem; }}
        ...
        [class*="st-key-html-table-interactive-row-"] [data-testid="stHorizontalBlock"] > div:last-child {{ align-items:center; display:flex; justify-content:center; margin:0!important; min-height:2.65rem; min-width:0; padding-left:0; padding-right:0; }}
        [class*="st-key-html-table-interactive-row-"] [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stVerticalBlock"], [class*="st-key-html-table-interactive-row-"] [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stButton"] {{ align-items:center; display:flex; justify-content:center; height:100%; margin:0!important; min-height:0; padding-bottom:0!important; padding-top:0!important; }}
        [class*="st-key-html-table-interactive-"] [data-testid="stButton"] {{ align-items:center; display:flex; justify-content:center; margin:0!important; min-height:0; width:100%; }}
        [class*="st-key-html-table-interactive-"] [data-testid="stButton"] button {{ align-items:center; background:transparent; box-shadow:none!important; color:var(--text-secondary); display:flex; font-size:.7rem; height:26px; justify-content:center; min-height:26px; min-width:32px; padding:0; white-space:nowrap; width:32px; }}
        [class*="st-key-html-table-interactive-"] [data-testid="stButton"] button:hover {{ background:var(--hover); border-color:var(--accent); box-shadow:none!important; color:var(--accent); transform:none; }}
        ...
        [class*="st-key-html-table-action-session-connections-"] [data-testid="stButton"] button {{ align-items:center; background:transparent; border:none!important; border-radius:6px; box-shadow:none!important; color:var(--accent); font-size:.82rem; height:28px; justify-content:center; line-height:1.2; min-height:28px; min-width:28px; outline:none; padding:0; width:28px; }}
        [class*="st-key-html-table-action-session-connections-"] [data-testid="stButton"] button svg {{ color:currentColor!important; fill:currentColor!important; height:18px; width:18px; }}
        [class*="st-key-html-table-action-session-connections-"] [data-testid="stButton"] button:hover {{ background:var(--action-hover); border:none!important; box-shadow:none!important; color:var(--accent); transform:none; }}
        [class*="st-key-html-table-action-session-connections-"] [data-testid="stButton"] button:focus, [class*="st-key-html-table-action-session-connections-"] [data-testid="stButton"] button:focus-visible {{ background:transparent; border:none!important; box-shadow:none!important; color:var(--accent); outline:none; }}
        [class*="st-key-html-table-action-"] *, [class*="st-key-html-table-interactive-row-"] [data-testid="stButton"] {{ box-shadow:none!important; filter:none!important; text-shadow:none!important; }}
        [class*="st-key-html-table-action-"] [data-testid="stElementContainer"], [class*="st-key-html-table-action-"] [data-testid="stTooltipHoverTarget"], [class*="st-key-html-table-action-"] [data-testid="stTooltipIcon"], [class*="st-key-html-table-action-"] [data-testid="stButton"], [class*="st-key-html-table-action-"] button[data-testid="stBaseButton-tertiary"] {{ background:transparent!important; box-shadow:none!important; filter:none!important; text-shadow:none!important; transform:none!important; }}
        [class*="st-key-html-table-action-"] [data-testid="stButton"] button {{ box-shadow:none!important; filter:none!important; transform:none; }}
        [class*="st-key-html-table-action-"] [data-testid="stButton"] button:hover {{ box-shadow:none!important; filter:none!important; transform:none; }}
        [class*="st-key-html-table-action-"]:not([class*="session-connections-"]) button[data-testid="stBaseButton-tertiary"], [class*="st-key-html-table-action-"]:not([class*="session-connections-"]) button[kind="tertiary"] {{ align-items:center; background:transparent!important; border:0!important; border-radius:6px; color:var(--text)!important; box-shadow:none!important; filter:none!important; min-height:26px; min-width:26px; padding:3px 6px!important; transform:none!important; }}
        [class*="st-key-html-table-action-"]:not([class*="session-connections-"]) button[data-testid="stBaseButton-tertiary"]:hover, [class*="st-key-html-table-action-"]:not([class*="session-connections-"]) button[kind="tertiary"]:hover {{ background:var(--input-hover)!important; border:0!important; box-shadow:none!important; color:var(--text)!important; transform:none!important; }}
        [class*="st-key-html-table-interactive-row-session-connections-"] [data-testid="stHorizontalBlock"] > div {{ align-items:center; }}
        #html-table-connected-quality-sources .html-table-cell {{ padding:5px .72rem; vertical-align:middle; }}
        #html-table-connected-quality-sources .html-table-cell--special {{ padding:5px .72rem; }}
        #html-table-connected-quality-sources th {{ padding:6px .72rem; vertical-align:middle; }}
        ...
        [class*="st-key-auth-card-"] {{ box-sizing:border-box; margin:0 0 0 auto; max-width:520px; padding:2.5rem 2.15rem 2rem; width:100%; }}
        ...
        .auth-card-eyebrow-row {{ align-items:center; display:flex; gap:.45rem; min-height:2rem; }}
        ...
        [class*="st-key-auth-card-topbar-"] {{ min-height:2rem; position:relative; }}
        [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] {{ align-items:center; background:transparent!important; background-color:transparent!important; border:0!important; box-shadow:none!important; display:flex; filter:none!important; height:36px; justify-content:center; margin:0!important; outline:0!important; padding:0!important; position:absolute; right:0; top:-.2rem; width:36px!important; z-index:1; }}
        [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] > [data-testid="stVerticalBlock"] {{ align-items:center; background:transparent!important; background-color:transparent!important; border:0!important; box-shadow:none!important; display:flex; filter:none!important; height:36px; justify-content:center; margin:0!important; outline:0!important; padding:0!important; width:36px!important; }}
        [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] [data-testid="stButton"], [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] [data-testid="stTooltipHoverTarget"] {{ align-items:center; background:transparent!important; background-color:transparent!important; border:0!important; box-shadow:none!important; display:flex; filter:none!important; height:36px; margin:0!important; min-height:36px; outline:0!important; padding:0!important; width:36px; }}
        [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] [data-testid="stButton"] button {{ align-items:center; background:transparent!important; background-color:transparent!important; border:none!important; border-radius:8px; box-shadow:none!important; color:var(--icon-secondary)!important; display:flex; filter:none!important; height:36px; justify-content:center; margin:0!important; min-height:36px; min-width:36px; outline:0!important; padding:0!important; width:36px; }}
        [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] [data-testid="stButton"] button:hover {{ background:var(--auth-toggle-hover)!important; background-color:var(--auth-toggle-hover)!important; border:none!important; box-shadow:none!important; color:var(--accent)!important; filter:none!important; }}
        [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] [data-testid="stButton"] button:active {{ background:var(--auth-toggle-hover)!important; background-color:var(--auth-toggle-hover)!important; border:none!important; box-shadow:none!important; color:var(--accent)!important; filter:none!important; }}
        [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] [data-testid="stButton"] button:focus-visible {{ background:transparent!important; background-color:transparent!important; border:none!important; box-shadow:none!important; outline:2px solid var(--accent)!important; outline-offset:1px; }}
        [class*="st-key-auth-card-topbar-"] [class*="st-key-auth-theme-control-"] [data-testid="stIconMaterial"] {{ color:currentColor!important; display:block; font-size:19px!important; height:19px!important; line-height:19px!important; margin:0!important; width:19px!important; }}
        ...
        /* Streamlit tooltips and notifications are portalled outside the widget tree. */
        [data-baseweb="tooltip"], [data-testid="stTooltip"], [data-testid="stTooltipContent"], [data-testid*="Tooltip"], [role="tooltip"] {{ background:var(--surface-elevated)!important; background-color:var(--surface-elevated)!important; border:1px solid var(--border)!important; border-radius:6px; box-shadow:0 8px 22px rgba(25,43,55,.18)!important; color:var(--text)!important; }}
        [data-baseweb="tooltip"] *, [data-testid="stTooltip"] *, [data-testid="stTooltipContent"] *, [data-testid*="Tooltip"] *, [role="tooltip"] * {{ background:transparent!important; color:var(--text)!important; fill:currentColor!important; stroke:currentColor!important; }}
        [data-testid="stToast"], [data-testid="stToastContainer"], [class*="stToast"], [role="alert"] {{ background:var(--surface)!important; background-color:var(--surface)!important; border:1px solid #4E9A62!important; border-radius:8px; box-shadow:none!important; color:var(--text)!important; padding:.55rem .7rem!important; }}
        [data-testid="stToast"] > div, [data-testid="stToastContainer"] > div, [class*="stToast"] > div {{ background:transparent!important; border:0!important; border-radius:0!important; box-shadow:none!important; padding:0!important; }}
        [data-testid="stToast"] *, [data-testid="stToastContainer"] *, [class*="stToast"] *, [role="alert"] * {{ color:var(--text)!important; fill:currentColor!important; stroke:currentColor!important; }}
        [data-testid="stToast"] button, [data-testid="stToastContainer"] button, [class*="stToast"] button {{ display:none!important; }}
        [class*="st-key-table-shell-"] [data-testid="stHorizontalBlock"], [class*="st-key-table-shell-"] [data-testid="stDownloadButton"], [class*="st-key-table-shell-"] [data-testid="stElementContainer"], [class*="st-key-table-shell-"] [data-testid="stTooltipHoverTarget"], [class*="st-key-table-shell-"] [data-testid="stTooltipIcon"], [class*="st-key-table-shell-"] button[data-testid="stBaseButton-tertiary"] {{ background:transparent!important; border:0!important; box-shadow:none!important; filter:none!important; transform:none!important; }}
        /* Authenticated header: keep the theme control as the fixed right anchor. */
        [class*="st-key-authenticated-header-row"] > [data-testid="stHorizontalBlock"] {{ flex-wrap:nowrap!important; }}
        [class*="st-key-authenticated-header-row"] > [data-testid="stHorizontalBlock"] > div:last-child {{ flex:0 0 25%!important; min-width:0!important; width:25%!important; }}
        [class*="st-key-header-actions"] {{ align-items:center; flex:0 1 100%!important; margin-left:auto!important; max-width:100%; min-width:0!important; width:100%!important; }}
        [class*="st-key-header-actions"] > [data-testid="stVerticalBlock"] {{ align-items:flex-end; max-width:100%; min-width:0!important; width:100%!important; }}
        [class*="st-key-header-actions"] [data-testid="stHorizontalBlock"] {{ flex-wrap:nowrap!important; justify-content:flex-end; max-width:100%; min-width:0!important; width:100%!important; }}
        [class*="st-key-header-actions"] [data-testid="stHorizontalBlock"] > div:first-child {{ flex:0 1 auto!important; min-width:0!important; width:max-content!important; }}
        [class*="st-key-header-actions"] [data-testid="stHorizontalBlock"] > div:last-child {{ flex:0 0 34px!important; min-width:34px!important; width:34px!important; }}
        [class*="st-key-authenticated-account-control"] {{ max-width:280px; min-width:120px; width:max-content!important; }}
        [class*="st-key-authenticated-account-control"] [data-testid="stHorizontalBlock"] > div:first-child {{ flex:0 1 auto!important; max-width:236px; min-width:78px; width:max-content!important; }}
        .authenticated-username {{ max-width:236px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }}
        @media (max-width:800px) {{
            [class*="st-key-authenticated-account-control"] {{ max-width:190px; }}
            [class*="st-key-authenticated-account-control"] [data-testid="stHorizontalBlock"] > div:first-child, .authenticated-username {{ max-width:146px; }}
        }}
        /* Keep the dropdown chevron centered within its existing fixed arrow section. */
        [class*="st-key-authenticated-account-control"] [data-testid="stHorizontalBlock"] > div:last-child {{ align-items:center!important; display:flex!important; justify-content:center!important; margin:0!important; padding:0!important; }}
        [class*="st-key-authenticated-account-control"] [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stPopover"] {{ align-items:center!important; display:flex!important; justify-content:center!important; margin:0!important; padding:0!important; width:42px!important; }}
        [class*="st-key-authenticated-account-control"] [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stPopover"] button {{ align-items:center!important; display:flex!important; flex-shrink:0!important; justify-content:center!important; margin:0!important; padding:0!important; }}
        [class*="st-key-authenticated-account-control"] [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stPopover"] button svg {{ flex-shrink:0!important; margin:0!important; }}
        .st-key-authenticated-account-control .authenticated-username {{ display:flex!important; align-items:center!important; justify-content:center!important; width:100%!important; height:100%!important; min-height:30px!important; line-height:1!important; margin:0!important; padding-top:0!important; padding-bottom:0!important; text-align:center!important; transform:none!important; }}
        .st-key-authenticated-account-control [data-testid="stMarkdownContainer"]:has(.authenticated-username), .st-key-authenticated-account-control [data-testid="stMarkdown"]:has(.authenticated-username), .st-key-authenticated-account-control [data-testid="stElementContainer"]:has(.authenticated-username) {{ align-items:center!important; display:flex!important; justify-content:center!important; width:100%!important; height:100%!important; min-height:30px!important; margin:0!important; padding:0!important; }}
        .st-key-authenticated-account-control [data-testid="stHorizontalBlock"] > div:last-child, .st-key-authenticated-account-control [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stElementContainer"], .st-key-authenticated-account-control [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stPopover"], .st-key-authenticated-account-control [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stPopoverButton"], .st-key-authenticated-account-control [data-testid="stHorizontalBlock"] > div:last-child [data-testid="stPopover"] button {{ align-items:center!important; display:flex!important; height:100%!important; justify-content:center!important; margin:0!important; padding:0!important; }}
        .st-key-authenticated-account-control [data-testid="stIconMaterial"] {{ display:flex!important; align-items:center!important; justify-content:center!important; width:16px!important; height:16px!important; line-height:16px!important; margin:0!important; padding:0!important; transform:none!important; }}
        .card-decoration-verified-path::before {{ border:1px solid color-mix(in srgb, var(--text-secondary) 7%, transparent); border-radius:50%; height:22px; right:18px; top:18px; width:22px; }} .card-decoration-verified-path::after {{ background:linear-gradient(145deg,transparent 42%,color-mix(in srgb, var(--accent) 10%, transparent) 43% 46%,transparent 47%),linear-gradient(25deg,transparent 57%,color-mix(in srgb, var(--text-secondary) 6%, transparent) 58% 59%,transparent 60%); bottom:20px; height:30px; right:8px; width:82px; }} .card-decoration-broken-path::before {{ background:linear-gradient(12deg,transparent 45%,color-mix(in srgb, var(--text-secondary) 6%, transparent) 46% 47%,transparent 48%),linear-gradient(12deg,transparent 58%,color-mix(in srgb, var(--text-secondary) 5%, transparent) 59% 60%,transparent 61%); height:48px; right:10px; top:24px; width:92px; }} .card-decoration-broken-path::after {{ background:radial-gradient(circle at 12% 54%,color-mix(in srgb, var(--text-secondary) 8%, transparent) 0 3px,transparent 4px),radial-gradient(circle at 49% 54%,color-mix(in srgb, var(--accent) 8%, transparent) 0 3px,transparent 4px),radial-gradient(circle at 88% 54%,color-mix(in srgb, var(--text-secondary) 8%, transparent) 0 3px,transparent 4px); height:48px; right:10px; top:24px; width:92px; }} .card-decoration-signal-path::before {{ background:linear-gradient(155deg,transparent 40%,color-mix(in srgb, var(--text-secondary) 6%, transparent) 41% 43%,transparent 44%),linear-gradient(25deg,transparent 58%,color-mix(in srgb, var(--text-secondary) 5%, transparent) 59% 61%,transparent 62%); height:52px; right:10px; top:18px; width:92px; }} .card-decoration-signal-path::after {{ background:radial-gradient(circle at 74% 48%,color-mix(in srgb, var(--accent) 10%, transparent) 0 4px,transparent 5px); height:52px; right:10px; top:18px; width:92px; }} .card-decoration-check-grid::before {{ background:linear-gradient(90deg,color-mix(in srgb, var(--text-secondary) 6%, transparent) 1px,transparent 1px),linear-gradient(color-mix(in srgb, var(--text-secondary) 6%, transparent) 1px,transparent 1px); background-size:16px 16px; height:48px; right:12px; top:20px; width:48px; }} .card-decoration-check-grid::after {{ background:radial-gradient(circle at 18% 18%,color-mix(in srgb, var(--accent) 9%, transparent) 0 3px,transparent 4px); height:48px; right:12px; top:20px; width:48px; }} .quality-empty-trend-art {{ background:linear-gradient(155deg,transparent 38%,color-mix(in srgb, var(--text-secondary) 5%, transparent) 39% 41%,transparent 42%),linear-gradient(25deg,transparent 58%,color-mix(in srgb, var(--text-secondary) 4%, transparent) 59% 61%,transparent 62%); bottom:16px; height:62px; opacity:.7; pointer-events:none; position:absolute; right:18px; user-select:none; width:110px; }} .quality-empty-trend-art::after {{ background:radial-gradient(circle at 10% 58%,color-mix(in srgb, var(--text-secondary) 8%, transparent) 0 3px,transparent 4px),radial-gradient(circle at 52% 40%,color-mix(in srgb, var(--accent) 9%, transparent) 0 3px,transparent 4px),radial-gradient(circle at 92% 27%,color-mix(in srgb, var(--text-secondary) 8%, transparent) 0 3px,transparent 4px); content:""; inset:0; position:absolute; }}
        [class*="st-key-quality-history-trend"] {{ overflow:hidden; position:relative; }} [class*="st-key-quality-history-trend"] > div {{ position:relative; z-index:1; }}
        @media (max-width:600px) {{ .quality-empty-trend-art {{ opacity:.55; transform:scale(.8); transform-origin:bottom right; }} }}
        .card-decoration-verified-path,.card-decoration-broken-path,.card-decoration-signal-path,.card-decoration-check-grid {{ overflow:hidden; position:relative; }} .card-decoration-verified-path::before,.card-decoration-verified-path::after,.card-decoration-broken-path::before,.card-decoration-broken-path::after,.card-decoration-signal-path::before,.card-decoration-signal-path::after,.card-decoration-check-grid::before,.card-decoration-check-grid::after {{ content:""; pointer-events:none; position:absolute; user-select:none; z-index:0; }}
        </style>
        """,
